...
const DashboardAdminKomunitasClient = ({ children }: { children: React.ReactNode; }) => {
  ...
  const fetchProduk = useCallback(async () => {
    ...
    if ("success" in result && result.success && result.data) setProdukList(result.data);
  ...
  return (
    <div className="container mx-auto py-6 px-4 sm:py-10">
      <div className="text-center mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold mb-2" style={{ color: "#4c7a6b" }}>
      ...
      <Tabs defaultValue="pesanan-masuk" className="w-full">
        <TabsList className="grid w-full grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-0 h-auto sm:h-10 bg-gray-50 p-2 rounded-lg">
          ...
          <TabsTrigger value="tambah-produk" className="flex items-center gap-2 text-xs sm:text-sm py-2 sm:py-1 px-3 data-[state=active]:bg-white data-[state=active]:shadow-sm transition-all duration-200 rounded-md" style={{ color: "#4c7a6b" }}>
            <Plus className="h-4 w-4" />
            <span className="hidden sm:inline">Ajukan Produk</span><span className="sm:hidden">Ajukan</span>
          </TabsTrigger>
        </TabsList>
            ...
            <CardContent className="space-y-0 divide-y divide-gray-200">
              ...
              produkList.length > 0 ? (
                produkList.map((produk) => (
                  <div key={produk.id} className="flex flex-col sm:flex-row sm:justify-between sm:items-center p-4 gap-3">
                    <div className="flex-1">
                      <p className="font-semibold text-gray-900 mb-1">{produk.judul}</p>
                      {produk.catatan_pemerintah && <div className="bg-amber-50 p-3 rounded-md border-l-4 border-amber-200 mt-2"><p className="text-sm text-amber-800"><strong>Catatan:</strong> {produk.catatan_pemerintah}</p></div>}
                    </div>
                    <div className="flex items-center gap-2 self-end sm:self-center">
                      <Badge variant={produk.status === "disetujui" ? "default" : produk.status === "ditolak" ? "destructive" : "secondary"}>{produk.status}</Badge>
              ...
              ) : ( <div className="text-center py-12"><Package className="mx-auto h-12 w-12 text-gray-400 mb-4" /><p className="text-gray-500 text-sm sm:text-base">Anda belum mengajukan produk apa pun.</p></div> )}
            </CardContent>
          </Card>
        </TabsContent>
